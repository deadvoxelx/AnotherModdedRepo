...
void FoxModel::render(shared_ptr<Entity> entity, float time, float r, float bob, float yRot, float xRot, float scale, bool usecompiled)
{
...
void FoxModel::prepareMobModel(shared_ptr<LivingEntity> mob, float time, float r, float a)
{
    ...
    float angle = fox->getHeadRollAngle(a) + fox->getBodyRollAngle(a, 0);
    head->zRot = angle;

    upperBody->zRot = fox->getBodyRollAngle(a, -.08f);
    body->zRot = fox->getBodyRollAngle(a, -.16f);
    tail->zRot = fox->getBodyRollAngle(a, -.2f);
}

void FoxModel::setupAnim(float time, float r, float bob, float yRot, float xRot, float scale, shared_ptr<Entity> entity, unsigned int uiBitmaskOverrideAnim)
{
