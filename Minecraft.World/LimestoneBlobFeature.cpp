...
LimestoneBlobFeature::LimestoneBlobFeature(int blockId) : Feature(blockId)
{
}

bool LimestoneBlobFeature::place(Level *level, Random *random, int x, int y, int z)
{
    return generate(level, random, x, y, z, 24);
}
  
bool LimestoneBlobFeature::generate(Level *level, Random *random, int x, int y, int z, int l)
{
    if (y - l <= 0) y = l + 1;
        ...
        for (int l2 = l; l2 >= -l; l2--)
        {
            for (int i3 = -l; i3 <= l; i3++)
            {
                ...
                if (Mth::sqrt(k3 * k3 + i4 * i4 + k4 * k4 + 0.0) <= l)
                    ...
                    {
                        placeBlock(level, i2 + x, l2 + y, i3 + z, Tile::limestone_Id, 0);
                        placeBlock(level, i2 + x, l2 + y - 1, i3 + z, Tile::limestone_Id, 0);
                        placeBlock(level, i2 + x, l2 + y - 1 + random->nextInt(2), i3 + z, Tile::limestone_Id, 0);
                    }
            } 
        } 
    } 
...
bool LimestoneBlobFeature::generateBlob(Level *level, Random *random, int x, int y, int z, int l)
{
    if (y - l <= 0) y = l + 1; 
        ...
        for (int k1 = l; k1 >= -l; k1--)
        {
            for (int i2 = -l; i2 <= l; i2++)
            {
                ...
                if (Mth::sqrt((k2 * k2 + i3 * i3 + k3 * k3 + 0.0)) <= l)
                    ...
                    {
                        placeBlock(level, i1 + x, k1 + y, i2 + z, Tile::limestone_Id, 0);
                        placeBlock(level, i1 + x, k1 + y - 1, i2 + z, Tile::limestone_Id, 0);
                        placeBlock(level, i1 + x, k1 + y - 1 + random->nextInt(2), i2 + z, Tile::limestone_Id, 0);
                    }
            }
        }
    } 
