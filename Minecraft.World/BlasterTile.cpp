...
bool BlasterTile::noDrop = false;

BlasterTile::BlasterTile(int id, bool lit) : BaseEntityTile(id, Material::stone)
{
	random = new Random();
	this->lit = lit;
...
int BlasterTile::getResource(int data, Random *random, int playerBonusLevel)
{
	return Tile::blaster_Id;
}

void BlasterTile::onPlace(Level *level, int x, int y, int z)
{
	BaseEntityTile::onPlace(level, x, y, z);
	recalcLockDir(level, x, y, z);
}

void BlasterTile::recalcLockDir(Level *level, int x, int y, int z)
{
...
Icon *BlasterTile::getTexture(int face, int data)
{
...
void BlasterTile::registerIcons(IconRegister *iconRegister)
{
	icon = iconRegister->registerIcon(L"blaster_side");
	iconFront = iconRegister->registerIcon(lit ? L"blaster_front_on" : L"blaster_front");
	iconTop = iconRegister->registerIcon(L"blaster_top");
}

void BlasterTile::animateTick(Level *level, int xt, int yt, int zt, Random *random)
{
	if (!lit) return;
...
bool BlasterTile::TestUse()
{
	return true;
}

bool BlasterTile::use(Level *level, int x, int y, int z, shared_ptr<Player> player, int clickedFace, float clickX, float clickY, float clickZ, bool soundOnly/*=false*/) // 4J added soundOnly param
{
...
void BlasterTile::setLit(bool lit, Level *level, int x, int y, int z)
{
	int data = level->getData(x, y, z);
	shared_ptr<TileEntity> te = level->getTileEntity(x, y, z);

	noDrop = true;
	if (lit) level->setTileAndUpdate(x, y, z, Tile::blaster_lit_Id);
	else level->setTileAndUpdate(x, y, z, Tile::blaster_Id);
	noDrop = false;
...
shared_ptr<TileEntity> BlasterTile::newTileEntity(Level *level)
{
	return std::make_shared<FurnaceTileEntity>();
}

void BlasterTile::setPlacedBy(Level *level, int x, int y, int z, shared_ptr<LivingEntity> by, shared_ptr<ItemInstance> itemInstance)
{
...
void BlasterTile::onRemove(Level *level, int x, int y, int z, int id, int data)
{
	if (!noDrop)
	{
...
bool BlasterTile::hasAnalogOutputSignal()
{
	return true;
}

int BlasterTile::getAnalogOutputSignal(Level *level, int x, int y, int z, int dir)
{
	return AbstractContainerMenu::getRedstoneSignalFromContainer(dynamic_pointer_cast<Container>( level->getTileEntity(x, y, z)) );
}

int BlasterTile::cloneTileId(Level *level, int x, int y, int z)
{
	return Tile::blaster_Id;
}